...
def generate(args):
   ...
   if "t2v" in args.task:
       ...
       def benchmark_generation(wan_i2v, args, img, MAX_AREA_CONFIGS):
           """
           Wraps the benchmarking logic:
           1. Runs 3 warm-up iterations and logs their times.
           2. Runs 5 measurement iterations, logs their times, and calculates the average.
           3. Returns the video generated from the final run.
           """


           def run_pass(seed, run_name):
               """
               An internal helper function to execute one generation pass and return its duration.
               Uses torch.cuda.synchronize() to ensure accurate GPU timing.
               """
               # (Critical!) Force the CPU to wait until the GPU has completed all previous work.
               torch.cuda.synchronize()
               start_time = time.perf_counter()


               # --- Run core generation code ---
               ...
               # (Critical!) Force the CPU to wait until the GPU has finished the "generate" task just submitted.
               torch.cuda.synchronize()
               end_time = time.perf_counter()
              
               duration = end_time - start_time
               logging.info(f"{run_name}: {duration:.2f} seconds")
              
               # Return the duration and the generated video
               return duration, video


           # --- 1. Warm-up Phase ---
           # Warm-up is necessary for the GPU to pre-load CUDA kernels and
           # reach a stable operating temperature, ensuring more accurate measurements.
           logging.info("--- Starting 3 Warm-up Runs ---")
           for i in range(3):
               # Use different seeds (e.g., +i) to ensure each run is "fresh"
               run_pass(seed=args.base_seed + i, run_name=f"Warm-up Run {i+1}/3")
           logging.info("--- Warm-up Complete ---")


           # --- 2. Measurement Phase ---
           logging.info("--- Starting 5 Measurement Runs ---")
           measurement_times = []
           final_video = None
          
           for i in range(5):
               # Also use different seeds
               duration, video = run_pass(seed=args.base_seed + 10 + i, run_name=f"Measurement Run {i+1}/5")
               measurement_times.append(duration)
              
               # Save the video from the last run for potential export
               if i == 4:
                   final_video = video
           ...
           average_time = np.mean(measurement_times)
           logging.info(f"\n--- Final Results ---")
           logging.info(f"Average time over 5 runs: {average_time:.2f} seconds")
          
           # Return the video from the last generation
           return final_video


       logging.info("Starting benchmark execution...")
       video = benchmark_generation(wan_i2v, args, img, MAX_AREA_CONFIGS)                   
